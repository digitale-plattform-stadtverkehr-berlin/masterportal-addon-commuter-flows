...
export class CommuterApi {
     ...
     * @returns {void} -
     */
    getListStates (onsuccess, onerror) {
        ...
        this.getFilterCall(url, {
            ...
            const parser = new DOMParser(),
                ...
                list = this.parseResponseGetFeature(doc, "euvm:ortsteile", "euvm:stadt", this.options.blacklistedDistricts);
    ...
    /**
     * returns a sorted list of available districts
     * @param {String} state the state to receive available districts from
     * @param {Function} onsuccess a function(list) with list as String[], called on success
     * @param {Function} onerror a function(error) with error as Error, called on error
     * @returns {void} -
     */
    getListDistricts (state, onsuccess, onerror) {
        const url = this.options.useProxy ? getProxyUrl(this.options.serviceUrl) : this.options.serviceUrl;
        ...
        this.getFilterCall(url, {
            ...
            const parser = new DOMParser(),
                doc = parser.parseFromString(xmlResponse, "application/xml"),
                list = this.parseResponseGetFeature(doc, "euvm:ortsteile", "euvm:ortsteil", this.options.blacklistedDistricts);
    ...
    getListCities (district, onsuccess, onerror) {
        ...
        this.getFilterCall(url, {
            featureType: "teilverkehrszellen",
            filter: [{
            ...
            const parser = new DOMParser(),
                doc = parser.parseFromString(xmlResponse, "application/xml"),
                list = this.parseResponseGetFeature(doc, "euvm:teilverkehrszellen", "euvm:teilverkehrszelle", this.options.blacklistedDistricts);
    ...
    getFeaturesDistrict (district, homeToWork, start, len, onsuccess, onerror) {
        ...
        this.getFilterCall(url, {
            featureType: homeToWork ? "auspendler" : "einpendler",
            filter: [{
                propertyName: homeToWork ? "euvm:wohnort" : "euvm:arbeitsort",
                literal: district
            }]
        }, xmlText => {
            const rawFeatureList = this.convertXmlToFeatureList(xmlText, homeToWork ? "auspendler" : "einpendler"),
    ...
    parseResponseGetFeature (xmlResponse, gmlApp, gmlItem, blacklist = []) {
        ...
        Object.keys(mainTags).forEach(tagKey => {
            ...
            if (result.indexOf(item.getElementsByTagName(gmlItem)[0].textContent) === -1) {
     ...
     * @param {String[]} filter the params to filter
     * @returns {String} the filter query
     */
    getFilter (filter) {
        let result = "<ogc:Filter>";

        if (filter.length > 1) {
            result += "<ogc:And>";
        }
        result += filter.map(f => Array.isArray(f) ? this.getOrFilter(f) : "<ogc:PropertyIsEqualTo><ogc:PropertyName>" + f.propertyName + "</ogc:PropertyName><ogc:Literal>" + f.literal + "</ogc:Literal></ogc:PropertyIsEqualTo>").join("");
        if (filter.length > 1) {
            result += "</ogc:And>";
        }
        result += "</ogc:Filter>";
    ...
    getOrFilter (filter) {
        ...
        result += filter.map(f => "<ogc:PropertyIsEqualTo><ogc:PropertyName>" + f.propertyName + "</ogc:PropertyName><ogc:Literal>" + f.literal + "</ogc:Literal></ogc:PropertyIsEqualTo>").join("");
        if (filter.length > 1) {
            result += "</ogc:Or>";
        }
...
<wfs:GetFeature service='${reqParams.service}' version='${reqParams.version}' xmlns:wfs='http://www.opengis.net/wfs' xmlns:ogc='http://www.opengis.net/ogc'>
    <wfs:Query typeName='${reqParams.featureType}' srsName='EPSG:25833'>
        ${this.getFilter(reqParams.filter)}
    </wfs:Query>
</wfs:GetFeature>`,
